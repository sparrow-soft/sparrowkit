...
# obliged to render does not erase the stored HMAC key. Stub that and the spec
# proves only that the controller calls a method.
module ConsoleCredentials
  ...
  # does now, because the passkey domain falls back to `sparrowkit: app_url` —
  # so a console example that sets an address silently rebinds every later
  # example's passkeys to it, and the suite fails or passes on the seed.
  #
  ...
  def stored
    forget!
    Rails.application.credentials.config[SparrowAuth::CREDENTIALS_KEY] || {}
  end
